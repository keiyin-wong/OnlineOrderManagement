...
<%
	...
	<main class='container' role='main'>
		<div class='container'>
			...
			BigDecimal creditLimit = (BigDecimal) request.getAttribute("creditLimit");
			int res = creditLimit.compareTo(total);
			%>
			<h2>Order Details</h2>
			<div class='row'>
				<div class='col-xs-12'>
					...
					<form class="simple_form form-horizontal order-edit-form "
						id="orderDetailForm" action="EmployeeOrderDetailsServlet"
							...
							<div class='col-xs-6'>
								<div class="form-group row string optional">
									<label class="string optional col-sm-4 control-label">Order
										Number</label>
									<div class="col-sm-8">
										<div class="string optional form-control"><%=order.getOrdernumber().toString()%></div>
										<input value=<%=order.getOrdernumber().toString()%>
								...
								<div class="form-group row string optional">
									...
									<div class="col-sm-8">
										<input value="<%=order.getOrderdate()%>"
											class="string optional form-control" type="date"
											name="orderDate">
									</div>
								</div>
								<div class="form-group row string optional">
									<label class="string optional col-sm-4 control-label">Required
										Date</label>
									<div class="col-sm-8">
										<input value="<%=order.getRequireddate()%>"
											class="string optional form-control" type="date"
											name="requiredDate">
									</div>
								</div>
								<div class="form-group row string optional">
									<label class="string optional col-sm-4 control-label">Shipped
										Date</label>
									<div class="col-sm-8">
										<input value="<%=order.getShippeddate()%>" id="shippedDate"
											class="string optional form-control" type="date" 
											name="shippedDate">
									</div>
								</div>
								<div class="form-group row string optional">
									<label class="string optional col-sm-4 control-label">Status</label>
									<div class="col-sm-8">
										<select id="status" name="status" 
											data-placeholder="Order status"
											class="form-control order_state-picker" form="orderDetailForm">
											<option value="Shipped" <%if((order.getStatus()).equals("Shipped")){%> selected <%}%> >Shipped</option>
											<option value="In Process" <%if((order.getStatus()).equals("In Process")){%> selected <%}%> >In Process</option>
											<option value="Cancelled" <%if((order.getStatus()).equals("Cancelled")){%> selected <%}%>>Cancelled</option>
											<option value="Resolved" <%if((order.getStatus()).equals("Resolved")){%> selected <%}%>>Resolved</option>
											<option value="Disputed" <%if((order.getStatus()).equals("Disputed")){%> selected <%}%>>Disputed</option>
											<option value="On Hold" <%if((order.getStatus()).equals("On Hold")){%> selected <%}%>>On Hold</option>
										</select>
										<div class="string optional"><p><b>Customer credit limit = RM
										<%
										out.println(creditLimit.toString());
										%>
										</b>
										</p>
										<%
										if(res == 0 || res == 1){
											out.println("<h4 style=\"color:green;\">Balance is sufficient</h4>");
										}else{
											out.println("<h4 style=\"color:red;\">Balance is insufficient</h4>");
										}
										%>
								...
								<div class="form-group row string optional">
									<label class="string optional col-sm-4 control-label">Comment</label>
									<div class="col-sm-8">
										<textarea class="string optional form-control"
											form="orderDetailForm" style="height: 200px" name="comments"><%=order.getComments()%></textarea>
									</div>
								</div>
								<div class="form-group row string optional">
									<label class="string optional col-sm-4 control-label">Customer
										Number</label>
									<div class="col-sm-8">
										<input class="string optional form-control" type="number" min = "100"
											value="<%=order.getCustomernumber()%>" name="customerNumber">
									</div>
								</div>
													...
													<table id="table" class='table table-striped table-hover'>
														...
														<tbody>
															...
															if (orderDetailsList.size() != 0) {
																...
																for (Orderdetail t : orderDetailsList) {
																	BigDecimal sum = t.getPriceeach().multiply(new BigDecimal(t.getQuantityordered()));
																	out.println("<tr>");
																	out.println("<td>" + t.getOrderlinenumber().toString() + "</td>");
																	out.println("<td>" + t.getId().getProductcode() + "</td>");
																	out.println("<td>" + t.getQuantityordered().toString() + "</td>");
																	out.println("<td>" + t.getPriceeach().toString() + "</td>");
																	out.println("<td>" + sum.toString() + "</td>");
																	out.println("</tr>");
																	total_1 = total_1.add(sum);
																}
																out.println("<tr>");
																out.println("<td></td>");
																out.println("<td></td>");
																...
																out.println("<td>" + total_1.toString() + "</td>");
																out.println("</tr>");
							...
							<div class='row'>
								<div class='col-xs-12'>
									<input type="submit" class="btn btn-primary"
										style="color: #428bca; border: 1px solid #ddd;" value="Edit">
	...
	<script>
		<%if((order.getStatus()).equals("In Process") || (order.getStatus()).equals("On Hold")){
		%>
			$(document).ready(function(){
				
				$('#orderDetailForm').submit(function() {
					var x = document.getElementById("shippedDate").value;
				      if($("#status").val() === "Shipped" && x === ""){
						alert("Please insert shipped date");
						return false;
					   }
					  if($("#status").val() === "Shipped" && <%= res%> == -1){
